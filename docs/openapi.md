...
frontendで使う型はOpenAPIから生成します。

```sh
pnpm --dir frontend generate-types
```
...
frontendでは `frontend/src/types/api.ts` によく使うschema型を再exportし、`frontend/src/services/api.ts` で `openapi-fetch` の共通clientを作ります。
...
現時点では、`EnumMap` 由来の `waitTimes`、stock、flavor config mapはfrontend側で明示型を補っています。
